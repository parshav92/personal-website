...
const SkillItem: React.FC<{ skill: typeof skills[0] }> = ({ skill }) => (
  <div className="flex items-center space-x-2 hover:scale-110 transform transition-transform duration-300">
    <span className="flex items-center bg-[hsl(0,0%,15%)]  px-4 py-2 rounded-full space-x-2 font-medium text-xs lg:text-base border border-white border-opacity-10 text-white">
...
const MarqueeRow: React.FC<{ skills: typeof skills; direction: 'normal' | 'reverse' }> = ({ skills, direction }) => {
  ...
  return (
    ...
    >
      <div 
        ref={rowRef}
        className={`flex lg:space-x-24 space-x-12 animate-marquee ${direction === 'reverse' ? 'animate-marquee-reverse' : ''}`}
